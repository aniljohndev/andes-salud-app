 ...
 */
public class fragmento_turnos_prestador extends Fragment implements View.OnClickListener{

    FragmentChangeTrigger trigger;
    private Dialog dialogo, dialogoMapa, dialogoMapaDatos;
    private float ultimoClick = 0, tiempoMulticlick = 2000;
    String idConvenioPrestador="", nombreConvenioPrestador="";
    LinearLayout btnAfiliado, btnCentroMedic, btnTurnos;
    LinearLayout afiliadoResultWrap, centroMedicResultWrap;
    ArrayList<HashMap<String, String>> listaProfesionalesMostrar = new ArrayList<HashMap<String, String>>();
    TextView nombrePrestador;
    String _idAfiliado, _nombreAfiliado, _idCentro, _nombreCentro, _domicilioCentro,
            _idHorario, _idProfesional,_idHora,_nombreMedico, _horaTurnoDesde, _idTurno, _coseguro, _diaSeleccionado
            , _horaSelecionada;
    String _desde, _hasta, _day;
    private Dialog dialog;


    public fragmento_turnos_prestador() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragmento_turnos_prestador, container, false);
    }
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        if (extras != null) {
            idConvenioPrestador = extras.getString("idConvenio", "");
            nombreConvenioPrestador = extras.getString("nombrePrestador", "");
        }
        nombrePrestador=(TextView) view.findViewById(R.id.nombrePrestador);
        nombrePrestador.setText(nombreConvenioPrestador);
        btnAfiliado = (LinearLayout)view.findViewById(R.id.btnAfiliado);btnAfiliado.setOnClickListener(this);
        btnCentroMedic = (LinearLayout)view.findViewById(R.id.btnCentroMedico);btnCentroMedic.setOnClickListener(this);
        btnTurnos = (LinearLayout)view.findViewById(R.id.btnTurnos);btnTurnos.setOnClickListener(this);

        afiliadoResultWrap = (LinearLayout)view.findViewById(R.id.afiliadoResultWrap);afiliadoResultWrap.setOnClickListener(this);
        centroMedicResultWrap = (LinearLayout)view.findViewById(R.id.centroMedicoResultWrap);centroMedicResultWrap.setOnClickListener(this);
    }
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.btnCentroMedico:
                if(_idAfiliado==null) {
                    dialog = fabrica_dialogos.dlgError(getActivity(),"Antes de seleccionar el centro debe seleccionar el afiliado");
                    dialog.show();
                    return;
                }
                leerCentros();
    ...
    void loadAfiliadoOptions(){
        try{
            dialog = fabrica_dialogos.dlgBuscando(getContext(), "Buscando el grupo familiar");
            dialog.show();
    ...
    private void leerCentros() {
        try{
            ...
            params.put("idProfesional", idConvenioPrestador);

            fabrica_WS.APPBuscarConsultoriosTurnosProfesional(getActivity(), params , new SuccessResponseHandler<Document>(){

                @Override
                public void onSuccess(Document valores) {
                    dialog.dismiss();

                    String[] idPoliconsultorio = new String[valores.getElementsByTagName("idPoliconsultorio").getLength()];
                    for (int i = 0; i < valores.getElementsByTagName("idPoliconsultorio").getLength(); i++) {
                        idPoliconsultorio[i] = valores.getElementsByTagName("idPoliconsultorio").item(i).getTextContent();
                    }
                    String[] policonsultorio = new String[valores.getElementsByTagName("policonsultorio").getLength()];
                    for (int i = 0; i < valores.getElementsByTagName("policonsultorio").getLength(); i++) {
                        policonsultorio[i] = valores.getElementsByTagName("policonsultorio").item(i).getTextContent();
                    }
                    ...
                    mostrarCentros(idPoliconsultorio, policonsultorio, direccion);
                }
            }, new ErrorResponseHandler(){
            ...
            ex.printStackTrace();
            dialog.dismiss();
            dialog = fabrica_dialogos.dlgError(getContext(), "Error desconocido al solicitar el centro con prestador:\n"+ex.getMessage());
            dialog.show();
        }
    }

    private void mostrarCentros(String[] identificador, String[] nombre, String[] extra) {
        try {
            if (identificador.length>=1) {
                ...
                for (int i = 0; i < identificador.length; i++) {
                    ...
                    pickerItems.add(new ItemPickerDialogFragment.Item(nombre[i], identificador[i], b));
                }
    ...
    private void leerTurnos() {
        try{
            ...
            params.put("idAfiliado", _idAfiliado);
            params.put("idProfesional", idConvenioPrestador);
            params.put("idCentro", _idCentro);

            fabrica_WS.APPBuscarTurnosLibresPrestadorCentro(getActivity(), params, new SuccessResponseHandler<Document>(){
                ...
                public void onSuccess(Document valores) {
                    ...
                    mostrarTurnos(idProfesional, idPrestacion, dia, hora, idHorario);
                }
            }, new ErrorResponseHandler(){
    ...
    private void mostrarTurnos(String[] idProfesional, String[] idPrestacion, String[] dia, String[] hora, String[] idHorario ) {
        try {
            if (idProfesional.length>=1) {
                ...
                for (int i = 0; i < idProfesional.length; i++) {
                    ...
                    map.put("profesional", idPrestacion[i]);
                    map.put("hora", hora[i]);
                    ...
                    if (!listaVerificar.contains(verificar)) {
                        Bundle b =new Bundle();
                        b.putString("hora", hora[i]);b.putString("idHorario", idHorario[i]);b.putString("dia", dia[i]);
                        pickerItemsProfMostrar.add(new ItemPickerDialogFragment.Item(idPrestacion[i], idProfesional[i], b));
                    }
                    listaVerificar.add(verificar);
                    listaProfesionalesMostrar.add(map);
                }
                _idProfesional=idPrestacion[0];
    ...
    private void auditarTurno(){
        try{
            ...
            params.put("idAfiliadoTitular", idAfiliadoTitular);
            params.put("idAfiliado", _idAfiliado);
            params.put("idPrestacion", _idProfesional);
    ...
    void showTurnosResultDialog(String valorDevuelto, String mensaje, String idTurno, String coseguro){
        ...
        TextView tvAfiliado = (TextView)dialog.findViewById(R.id.tvAfiliado);tvAfiliado.setText(_nombreAfiliado);
        TextView tvEspecialidad=(TextView)dialog.findViewById(R.id.tvEspecialidad);tvEspecialidad.setText(nombreConvenioPrestador);
        TextView tvCentroMedico=(TextView)dialog.findViewById(R.id.tvCentroMedico);tvCentroMedico.setText(_nombreCentro);
        TextView tvTurno1 = (TextView)dialog.findViewById(R.id.tvTurno1);tvTurno1.setText(String.format("%s hs", _horaSelecionada));
        TextView tvTurno2 = (TextView)dialog.findViewById(R.id.tvTurno2);tvTurno2.setText(_diaSeleccionado);
        TextView tvTurno3 = (TextView)dialog.findViewById(R.id.tvTurno3);tvTurno3.setText(_nombreMedico);
        TextView tvTurno4 = (TextView)dialog.findViewById(R.id.tvTurno4);tvTurno4.setText(String.format("Coseguro $%s", _coseguro));
        ...
        try{
            ...
            fabrica_WS.APPConfirmarTurnoAUT(getActivity(), params, new SuccessResponseHandler<Document>(){
                ...
                public void onSuccess(Document valores) {
                    ...
                    String resultado= database.agregarTurnoAndes(_idTurno, _nombreAfiliado, ""
                    ...
                    if (!resultado.equals("")){
                        ...
                        dialog.setOnDismissListener(new DialogInterface.OnDismissListener() {
                            public void onDismiss(final DialogInterface dialog) {
                                trigger.fireChange("turnoPrestador_btnTermino");
                                return;
                            }
                        });
                    }
